...
const page = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    school: "",
    tracks: "",
    howDidYouHearAboutUs: { platfrom: "", text: "" },
  });
  ...
  const handleSelectChange = (event) => {
    ...
    const selectedOption = howDidYouHearAboutUsOptions.find(
    ...
    setFormData((prevFormData) => ({
      ...
      howDidYouHearAboutUs: {
        ...
        text:
          selectedOption && selectedOption.textRequired
            ? prevFormData.howDidYouHearAboutUs.text
            : "",
      },
    }));
  };
  ...
  const handleCheckboxChange = (e) => {
    ...
    setFormData((prevState) => ({
      ...prevState,
      tracks: checked
        ? [...prevState.tracks, value]
        : prevState.tracks.filter((track) => track !== value),
    }));

   
  };

  const handleMentorConfirmationChange = (e) => {
    setFormData({ ...formData, mentorConfirmation: e.target.checked });
  };
  ...
  const handleInputChange = (e, idx, field, isMentor = false) => {
    if (isMentor) {
      setFormData({
        ...formData,
        mentor: { ...formData.mentor, [field]: e.target.value },
      });
    } else {
  ...
  const checkFormCompletion = () => {
    const areParticipantsFilled = formData.participants.every(
      (participant) =>
        participant.name != "" &&
        participant.class != "" &&
        participant.phone != "" &&
    ...
    const isMentorFilled =
      formData.mentor.name != "" &&
      formData.mentor.email != "" &&
      formData.mentor.phone != "";
    ...
    const isFormComplete =
      selectedEvent != "" &&
      selectedNationality != "" &&
      formData.tracks.len!=0 &&
      formData.howDidYouHearAboutUs.platfrom!=""&&
      formData.howDidYouHearAboutUs.text!=""
      areParticipantsFilled &&
      isSchoolSectionFilled &&
      isMentorFilled;
  ...
  return (
          ...
          <div className="mb-8 w-full">
            ...
            {formData.participants.map((participant, idx) => (
              <div key={idx} className="mb-6">
                <h3 className="text-lg text-blue-800 font-medium mb-4">
                  Student {idx + 1}
                </h3>
            ...
            <div className="flex flex-wrap gap-4">
              <div className="w-full md:w-1/3">
                ...
                <div className="flex flex-col">
                  {tracksInterested.map((track,idx) => (
                    <label key={idx} className="inline-flex items-center">
                      ...
                      />
                      <span className="ml-2 text-blue-800">{track}</span>
                    </label>
                  ))}
                </div>
              </div>
              <div>
              <label className="block text-blue-800 font-medium mb-2 md:mb-0 md:ml-1">
    How did you hear about us?
              ...
              <div className="md:flex h-10">
                ...
                <select
                  disabled={isFormDisabled}
                  value={formData.howDidYouHearAboutUs.platform}
                  onChange={handleSelectChange}
                  className="block w-full p-2 border rounded-md text-blue-800 bg-white"
                >
                  <option value="">Select</option>
                  {howDidYouHearAboutUsOptions.map((option,idx) => (
                    <option key={idx} value={option.platform}>
                      {option.platform}
                    </option>
                  ))}
                </select>

                {formData.howDidYouHearAboutUs.platform &&
                howDidYouHearAboutUsOptions.find(
                  (option) =>
                    option.platform === formData.howDidYouHearAboutUs.platform
                ).textRequired ? (
                  <input
                    type="text"
                    value={formData.howDidYouHearAboutUs.text}
                    onChange={(e) =>
                      setFormData((prevFormData) => ({
                        ...prevFormData,
                        howDidYouHearAboutUs: {
                          ...prevFormData.howDidYouHearAboutUs,
                          text: e.target.value,
                        },
                      }))
                    }
                    placeholder="Please specify"
                    className="block w-full md:ml-2 mt-2 md:mt-0 h-10 p-2 border rounded-md"
                  />
                ) : null}
              </div>
              </div>
          ...
          {/* Mentor Confirmation */}
          <div className="mb-8 md:mt-0 mt-8 w-full">
            <label className="inline-flex items-center">
              <input
                type="checkbox"
                disabled={isFormDisabled}
                checked={formData.mentorConfirmation}
                onChange={handleMentorConfirmationChange}
                className="form-checkbox h-4 w-4 text-blue-600"
              />
              <span className="ml-2 text-blue-800">
                I confirm that the mentor is aware and has approved the team's
                registration
              </span>
            </label>
          </div>
          ...
          <div className="mb-8 w-full">
            <label className="block text-blue-800 font-semibold mb-2">
              Upload Document (jpg, pdf, jpeg, png - max 5MB)
            </label>
            <input
              type="file"
          ...
          <div className="flex">
            ...
            <button
              ...
              className={`px-6 py-3 bg-blue-600 mx5 text-white font-semibold rounded-md hover:bg-blue-700 ${
                isButtonDisabled ? "opacity-50 cursor-not-allowed" : ""
              }`}
              disabled={isButtonDisabled}
            >
